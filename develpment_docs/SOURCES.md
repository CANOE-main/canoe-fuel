...
| F2 | NREL ATB Electricity Sector 2022 | Fuel costs for biomass, wood, and uranium (natural and enriched); assumed constant across all model periods | Config parameters `b_price`, `u_price` | `input/params.yaml` |
| F3 | Wolinetz & Harrison, *Biofuels in Canada 2023*, Navius Research | Canadian ethanol and renewable diesel prices; accounts for biofuel transportation costs in Ontario; average of 2012–2022, no time projection | Fixed prices in `inflation` sub-model | `input/params.yaml` |
...
- EIA AEO prices are US national data in `2024 $/MMBtu`. The pipeline converts to 2020 CAD M$/PJ using a currency factor (1.22 USD/CAD) and GDP deflators defined in `input/params.yaml` under `inflation:`.
